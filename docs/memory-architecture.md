...
本设计采用 MongoDB 作为统一记忆与状态存储。MongoDB 同时承载完整课堂记录、结构化长期记忆、LangGraph/DeepAgents 短期 checkpoint、课程状态、抽取任务和向量检索索引，减少早期中间件复杂度。当前产品决策是：长短期记忆、课程状态、checkpoint、RAG 和后续向量检索都优先放在 MongoDB；PostgreSQL 只用于用户、账号、权限、班级、教师/志愿者和运营关系等关系型产品数据。
...
- 物理存储统一在 MongoDB，逻辑模型必须分层。
- PostgreSQL 不承载课堂历史、短期记忆、长期记忆、checkpoint、RAG chunks 或向量索引。
...
### 3. 短期状态与 Checkpoint 层

保存 DeepAgents / LangGraph 线程内短期状态，用于恢复本节课当前上下文、工具调用中间结果、自动压缩摘要和下一步教学动作。
...
```json
{
  ...
  "current_section_id": "sec-positive-negative-numbers",
  ...
  "current_question": "如果收入 10 元记作 +10，支出 6 元应该怎么记？",
  "pending_student_action": "回答当前诊断题，并说明理由。",
  "teacher_style": "严格但温暖",
...
- `{ student_id: 1, current_knowledge_point_id: 1, updated_at: -1 }`

### `langgraph_checkpoints`

LangGraph/DeepAgents 的短期线程状态。`thread_id` 应使用 `lesson_sessions._id`，确保每节课可以恢复同一个 agent 线程。
...
```json
{
  ...
  "checkpoint_ns": "",
  "parent_checkpoint_id": "checkpoint_...",
  "checkpoint": {
    "messages": [],
    "lesson_state": {
      "current_phase": "diagnostic_check",
      "current_chapter_id": "ch1",
      "current_knowledge_point_id": "kp-positive-negative-numbers",
      "current_skill_id": "opent-teacher-rj-junior-math-grade7-vol1-kp-positive-negative-numbers",
      "current_question": "如果收入 10 元记作 +10，支出 6 元应该怎么记？",
      "student_answer_status": "awaiting_answer",
      "next_teaching_action": "evaluate_student_answer"
    },
    "selected_skill": {},
    "retrieved_chunks": [],
    "retrieved_memory": []
...
建议使用 LangGraph MongoDB checkpointer 的 collection 约定；如果库版本要求不同字段，以官方 saver 为准，但必须保持 `thread_id=session_id`。

### `langgraph_checkpoint_writes`

保存 checkpoint 过程中的中间 writes。该 collection 由 LangGraph checkpointer 管理，业务代码不应直接读写，除非做调试或迁移。
...
## DeepAgents / LangGraph 接入

项目主 agent 框架采用 LangChain DeepAgents / LangGraph。OpenTeacher 应把 DeepAgents / LangGraph 作为课堂主 harness runtime，而不是 provider-style prompt 的可选旁路。数据库细节不能暴露给 agent prompt，必须通过受控工具访问。

`session_id` 应作为 LangGraph `thread_id`。短期记忆由 MongoDB checkpointer 保存；长期记忆由 `memory_cards` 等 collection 保存；两者都在 MongoDB，但逻辑上必须分层。
...
- `select_teaching_skill_tool`
- `retrieve_textbook_chunks_tool`
- `retrieve_student_memory_tool`
- `evaluate_student_answer_tool`
...
DeepAgents 的规划能力适合主动授课流程，但安全、隐私、记忆写入和课程状态变更必须由工具和服务层约束。
...
8. 接入 LangGraph MongoDB checkpointer，`thread_id=session_id`。
9. 用 DeepAgents / LangGraph 替换当前 provider-style `AgentHarness` 主路径，provider 只作为 graph 内部 LLM executor。

## 暂不做

- 不把长短期记忆、checkpoint、课程状态、RAG 或向量索引绑定到 PostgreSQL。
- 不引入 Qdrant 作为第一阶段必需依赖；后续向量检索优先使用 MongoDB Atlas Vector Search。
- 不把每轮聊天都直接写成长期记忆。
